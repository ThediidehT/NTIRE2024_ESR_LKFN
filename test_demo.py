...
def select_dataset(data_dir, mode):
    ...
    # inference on the LSDIR_DIV2K_valid set
        ...
        path = [
            (p.replace("_HR", "_LR").replace(".png", "x4.png"), p)
            for p in sorted(
                glob.glob(os.path.join(data_dir, "LSDIR_DIV2K_valid_HR/*.png"))
            )
        ]
    else:
...
def main(args):
    ...
    utils_logger.logger_info(
        "NTIRE2024-EfficientSR", log_path="NTIRE2024-EfficientSR.log"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--include_test", action="store_true", help="Inference on the DIV2K test set"
    )
